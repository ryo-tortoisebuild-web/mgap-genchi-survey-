...
## メンバー（複数人）の追加
2人目以降は **setup_key を使いません**。管理者がアプリ内から追加します。

1. 管理者（最初に登録した人）でログイン → ヘッダーの「👥 メンバー」
2. ID とパスワード（6文字以上）を入力して「この内容で追加する」
3. 発行した ID / パスワードを本人に安全な方法で伝える

- 追加された人は **全ての物件を作成・閲覧・編集できます**（全員フル権限＝方式A）。
- **メンバーの追加・削除・パスワード再発行は管理者のみ**です。
  必要なら追加時に「この人も管理者にする」をONにすると管理者を増やせます。
- パスワード再発行するとその人の既存ログインは無効になり、再ログインが必要です。
- メンバーを削除しても、その人が作った物件データは共有資産として残ります。
- 管理者が0人になる削除・自分自身の削除はできません。

> DBの `users.is_admin` 列は初回アクセス時に自動追加され、
> 既存の最初のユーザーが自動的に管理者になります（手動SQL不要）。
